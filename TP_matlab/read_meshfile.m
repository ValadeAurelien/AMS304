...
% 
% SYNOPSIS [Nbpt,Nbtri,Coorpts,Refpts,Numtri,Reftri,Nbaretes,Numaretes,Refaretes]=lecmail(nomfile)
%          
% INPUT * nom_maillage : le nom d'un fichier de maillage au format msh
%                   AVEC SON SUFFIXE .msh
%
% OUTPUT - Nbpts : nbre de sommets (entier) 
%        - Nbtri : nbre de triangles (entier)
%        - Coorpts : coordonnees (x, y) des sommets (matrice reelle Nbpt x 2)
%        - Refpts : reference des sommets (vecteur entier Nbpts x 1)
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Nbpts,Nbtri,Coorpts,Refpts,Numtri,Reftri,Nbaretes,Numaretes,Refaretes]=read_meshfile(nomfile)
fid=fopen(nomfile,'r');
if fid <=0,
...
while skip
    line=fgetl(fid);
    skip = ~(strcmp(line, '$Nodes') || strcmp(line, 'Vertices'));
end
Nbpts = str2num(fgetl(fid));
Coorpts = zeros(Nbpts,2);
Refpts = zeros(Nbpts,1);
Nbaretes = 0;
Numaretes = [];
Refaretes = [];
RefptsBis = zeros(Nbpts,1);
for i=1:Nbpts
...
while skip
    line=fgetl(fid);
    skip = ~(strcmp(line, '$Elements') || strcmp(line, 'Triangles'));    
end
